...
module.exports = function (app, appExpress) {
    ...
    if (skipLogin) {
        ...
        req.session.user = fakeUser;  // Assign user to the session
        
        // Redirect the user to the dashboard or home page
        return res.redirect(base + '/');  // Replace '/dashboard' with your desired page
    }
    
    // Normal login logic (only executed if skipLogin is false)
    if(req.body.captch === req.body.captchtext) {
        var options = {
            'method': 'POST',
            'url': urlapi + '/addlogin',
        ...
        request(options, function (error, response) {
            ...
            if(data.status === 'success') {
                // Create a user object to be saved in the session
                let user = {
                    id: 1,
                    username: data.datam.username,
                    password: data.datam.password,
                    email: data.datam.email
                };

                req.session.user = user;  // Save user to session
                res.redirect(base + '/');  // Redirect to dashboard/home page
            } else {
                req.flash('error', 'Incorrect email or password!');
                res.redirect(base + '/login');
            }
